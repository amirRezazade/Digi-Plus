...
export default function ChatBottom({ message, onMessage, sendMessage, sendImg }) {
  ...
  return (
    <div className="flex items-center gap-1 relative px-2 py-1">
      ...
      <div className="grow">
        <input dir="rtl" ref={inputRef} value={message} onChange={(e) => onMessage(e.target.value)} onKeyUp={(e) => (e.key === "Enter" ? sendMessage() : "")} className="border-0 outline-0 w-full rounded-2xl px-3 py-2 bg-light-gray/40 text-dark " type="text" placeholder="پیام خود را وارد کنید" />
      </div>
