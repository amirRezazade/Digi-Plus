...
export default function SupportCat() {
  ...
  return (
    <div className={`fixed bottom-3 right-3 sm:bottom-8 sm:right-8 text-sm z-1  transition-all duration-300 overflow-hidden `}>
      <div className={`flex flex-col justify-between bg-white gray-shaddow rounded-2xl  overflow-hidden  w-70 h-100 translate-x-1/1 transition-all duration-400 opacity-0 invisible ${openChat ? "translate-x-0! visible! opacity-100!" : ""}`}>
        ...
        <div className="flex items-center gap-1 px-2 py-1">
          <button className="size-10 gradient rounded-full">
      ...
      {/* open close btn  */}
      <button onClick={() => setOpenChat(!openChat)} className={`cursor-pointer mt-1 size-13 flex justify-center items-center gradient rounded-full  transition-all duration-500 `}>
        <span>
          <svg fill="white" height="33" width="33" version="1.1" id="Icons" viewBox="0 0 32 32">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
