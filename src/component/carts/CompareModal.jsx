...
export default function CompareModal() {
  ...
  return (
    <div onMouseDown={(e) => e.target.classList.contains("modal-bg") && close()} className={`modal-bg hidden-scrollbar relative transition-all duration-500 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
      <div className={` min-w-150 min-h-100 md:w-full md:max-w-[750px] md:min-h-120 lg:max-w-[850px] 2xl:max-w-[950px] absolute top-1/2  -translate-y-1/2  bg-white text-gray p-3 rounded-2xl grid grid-cols-13 items-stretch  divide-x divide-light-gray  transition-all duration-500  ${isOpen ? "opacity-100! visible!" : "-rotate-y-100! opacity-0  invisible"} `}>
