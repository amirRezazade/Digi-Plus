...
export default function Tags({ tags }) {
  return (
    <div className="flex items-center gap-3 text-gray p-4 border gray-shaddow border-light-gray/70 rounded-2xl overflow-x-auto hidden-scrollbar">
      ...
      {tags.map((tag) => (
        <Link key={tag} to={`/shop/tag=${tag}`} class="shrink-0 px-3 py-2 rounded-full border border-light-gray flex items-center gap-1 text-sm shadow-[0px_4px_8px_0px_#6666661a] transition-colors duration-300 hover:text-red  hover:bg-light hover:border-org/30 hover:shadow-[0px_4px_8px_0px_#dc2e0217]">
