    ...
    <div className="flex flex-col justify-between px-4 py-6 mt-4 gap-5 text-gray text-sm shadow-[0px_2px_15px_0px_#3e3e3e1a] border border-light-gray/60 rounded-2xl ">
      ...
      <div className="text-end">
        ...
        <div className="flex justify-between items-center my-3">
          ...
          <div className="w-30 h-10 flex border border-light-gray rounded-lg">
            <button onClick={() => quantity < data.stock && setQuantity((prev) => prev + 1)} disabled={quantity == data.stock || data.stock == 0} className="cursor-pointer fill-red px-3 h-full disabled:fill-light-gray disabled:cursor-default">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                <rect x="2" y="7" width="12" height="2" rx="1"></rect>
                <rect x="7" y="14" width="12" height="2" rx="1" transform="rotate(-90 7 14)"></rect>
              </svg>
            </button>
            <span className="grow flex justify-center items-center px-2 h-full">{data.stock == 0 ? "0" : quantity}</span>
            <button onClick={() => quantity > 1 && setQuantity((prev) => prev - 1)} disabled={quantity == 1} className="cursor-pointer fill-red px-3 h-full disabled:fill-light-gray disabled:cursor-default">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                <rect x="2" y="7" width="12" height="2" rx="1"></rect>
              </svg>
            </button>
        ...
        <div className="mb-1.5">
          <span className="text-xs px-1 sm-shaddow gradient rounded text-white">%{data.discountPercentage}</span>
          <span className="text-gray/80 line-through text-sm mx-2">${calcRealPrice(data.price, data.discountPercentage)}</span>
        </div>
        <p className="text-xl md:text-2xl lg:text-3xl text-red font-bold mt-0.5">{formatedPrice(data.price)} $</p>
