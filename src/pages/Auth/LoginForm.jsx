...
export default function LoginForm({ onForm, form }) {
  ...
  return (
    <div className={` absolute top-1/2 -translate-y-1/2 right-0 w-full px-3 xs:px-10! md:px-3! lg:px-10! ${form !== "login" ? "scale-30 opacity-0 pointer-events-none" : ""} transition-[scale_opacity] duration-500`}>
      ...
      <p className="text-center">وارد حساب کاربریت شو و خریدت رو از دیجی پلاس ادامه بده.</p>
      <form className="mt-7 xs:mt-15 " onSubmit={(e) => submit(e)}>
        <div className="">
          <label htmlFor="phone-user-name" className="text-dark">
            نام کاربری یا شماره تلفن<span className="text-red">*</span>
          </label>
          <input type="text" id="phone-user-name" className="block w-full my-3 p-3 outline-0 gray-shaddow border border-light-gray/50 focus:border-org rounded-lg" required minLength={8} />
        </div>
        <div className="my-4">
          <label htmlFor="password" className="text-dark">
            رمز عبور <span className="text-red">*</span>
          </label>
          <div className="flex justify-between items-center gap-2 w-full my-3 p-3 gray-shaddow border border-light-gray/50 focus-within:border-org rounded-lg">
            <input type="password" id="password" className="grow w-full  outline-0 border-0" required minLength={6} />
